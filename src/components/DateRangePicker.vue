      ...
      .mj-calendar(:class="weekSelector ? 'mj-calendar-week' : 'mj-calendar-days'" v-if="isDaysPicker")
        .calendar-header
          .calendar-previous-month.calendar-arrow.calendar-arrow-previous(
            ...
            @click="changeMonth(1)"
            :class="(past || (!past && currentMonth > nowMonth && currentYear === nowYear ) || (!past && nowYear < currentYear)) ? 'click-allowed' : 'click-not-allowed'"
          )
          ...
          .calendar-previous-month.calendar-arrow.calendar-arrow-next(
            ...
            @click="changeMonth(-1)"
            :class="(future || (!future && currentMonth < nowMonth && nowYear === currentYear ) || (!future && nowYear > currentYear)) ? 'click-allowed' : 'click-not-allowed'"
          )
